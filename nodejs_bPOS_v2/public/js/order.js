...
const add_order_list = (order_list) => {
  ...
  // 기존의 div_order_list 가 있는지 확인하고 삭제
  // div_order_list를 가져와서, 전체를 article.order_list로 부터
  // 삭제하기
  ...
  const orders = order_list.map((order, index) => {
    ...
    menu_name.innerText = order.tbl_product2.p_name;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 아래의 event 핸들러는 div.button_box 가 만들어지기 전에
  // 선언되고 OS에게 알려진 코드이다
  // div.button_box가 아직 만들어지지 않는 상태에서
  ...
  document.addEventListener("click", (e) => {
    ...
    if (button.tagName === "BUTTON") {
      if (button.className.includes("btn_cash")) {
        document.querySelector("span.pay_qty").innerText = " 현금";
        modal.style.display = "flex";
      } else if (button.className.includes("btn_card")) {
        document.querySelector("span.pay_qty").innerText = " 카드";
        modal.style.display = "flex";
      }
