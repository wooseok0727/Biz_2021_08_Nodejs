...
router.get("/order/:table_id", async (req, res) => {
  ...
  const MENU = await tbl_product2s.findAll().then({ order: ["p_name", "ASC"] });
 ...
 *  })
 *
 *  JS에서 이러한 상황을 Callback Hell 이라고 한다
 *
 * promise 방식
 * async await
 */
router.get("/order/:table_id/input/:menu_id", async (req, res) => {
  ...
  // 선택된 메뉴를 menu_list 에 추가
  // await 가 부착된 함수는
  // 함수의 실행이 완료되고 menu 변수에 결과값이 담기기 전에는
  // 다음 코드로 진행이 안된다.
  const menu = await tbl_product2s.findByPk(menu_id);

  // 선택된 상품이 order list에 있는지를 검사하기 위해서
  // table-orders 에서 데이터 select 하기
