...
const { tbl_bbs, tbl_replys } = require("../models/index");

// 설정된 /write get는
// URL에서 localhost:3000/bbs/write 요청 할 때
// 응답할 함수
...
router.get("/detail", (req, res) => {
  ...
  // tbl_replys 의 r_postId = b_id 로 WHERE 를 실행하여
  // tbl_replay를 SELECT 하고 그 list를 함께 묶어서 결과로 달라
  tbl_bbs
    .findOne({
      where: { b_id },
      include: { model: tbl_replys },
    })
    .then((result) => {
      res.render("detail", { BBS: result });
    });
...
router.post("/replys", (req, res) => {
  tbl_replys.create(req.body).then((result) => {
 ...
 *
 * 3. pathVarriable
 * http://localhost:3000/bbs/replys/delete/3 이렇게 보낼때
 *
 * router.get("/replys/delete/:rid") 로 URL path를 설정하고
 * req.params.rid로 데이터를 받는다
 * const rid = req.params.rid 와 같이 받는다
 */

router.get("/replys/delete/:rid", (req, res) => {
  const rid = req.params.rid;

  tbl_replys.findByPk(rid).then((result) => {
    const postId = result.r_postId;
    tbl_replys.destroy({ where: { id: rid } }).then(() => {
